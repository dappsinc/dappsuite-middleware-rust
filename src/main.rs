...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/send-euroc", post(send_euroc));
...
// Basic Function to Send USDC
async fn send_euroc(Json(payload): Json<SendUsdc>) -> Result<T, E> {
    ...
    let euroc_token_addr = "0x1aBaEA1f7C830bD89Acc67eC4af516284b1bC33c".parse::<Address>()?;
    ...
    let signer = Arc::new(SignerMiddleware::new(provider, wallet.with_chain_id(chain_id.as_u64())));
    let contract = ERC20Contract::new(euroc_token_addr, signer);
